import {
  Box,
  Button,
  Flex,
  FormControl,
  FormLabel,
  Heading,
  Input,
  InputGroup,
  InputLeftElement,
  Select,
  Slider,
  SliderFilledTrack,
  SliderThumb,
  SliderTrack,
  Text,
} from "@chakra-ui/react";
import Head from "next/head";
import { ChevronLeft, ChevronDown } from "react-feather";

import GoBack from "components/GoBack";
import withHeader from "components/Header";
import { ExchangeType } from "lib/exchange";
import { FormEvent, FormEventHandler, useMemo, useState } from "react";
import Hg from "components/Hourglass";

function CreateExchange() {
  const menuItems = useMemo(
    () =>
      Object.values(ExchangeType)
        .filter((x) => isNaN(parseInt(x)))
        .map((type) => {
          const displayType = type.replace("_", " ");
          return <option key={type}>{displayType}</option>;
        }),
    []
  );
  const [sliderValue, setSliderValue] = useState(1);
  const [location, setLocation] = useState<null | string>(null);
  const [category, setCategory] = useState<null | ExchangeType>(null);

  const submitForm = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const description = e.target[0].value;
    console.log({ description, location, category, sliderValue });
  };

  return (
    <Box>
      <Head>
        <title>Create an Exchange</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Box mt="40px" ml="85px">
        <GoBack />
        <Heading mt="28px" fontSize="30px">
          Create an Exchange
        </Heading>
        <form onSubmit={submitForm}>
          <FormControl mt="20px" isRequired>
            <FormLabel fontSize="18px" fontWeight="light">
              Description
            </FormLabel>
            <InputGroup size="lg" maxWidth="30%" name="description">
              <InputLeftElement pointerEvents="none" ml="11px">
                <Text fontWeight="bold">I will </Text>
              </InputLeftElement>
              <Input placeholder="do something awesome..." paddingLeft="60px" />
            </InputGroup>
          </FormControl>
          <FormControl isRequired mt="30px">
            <FormLabel fontSize="18px" fontWeight="light">
              Category
            </FormLabel>
            <Select
              placeholder="Select Category"
              width="30%"
              size="lg"
              onChange={(e) => {
                setCategory(ExchangeType[e.target.value]);
              }}
            >
              {menuItems}
            </Select>
          </FormControl>
          <FormControl isRequired mt="30px">
            <FormLabel fontSize="18px" fontWeight="light">
              Length
            </FormLabel>
            <Flex width="30%" align="center">
              <Slider
                defaultValue={1}
                min={0.5}
                max={2}
                step={0.25}
                onChange={setSliderValue}
                flexGrow="1"
              >
                <SliderTrack bg="blue.100">
                  <Box position="relative" right={10} />
                  <SliderFilledTrack bg="blue.500" />
                </SliderTrack>
                <SliderThumb boxSize={6} bg="gray.300" />
              </Slider>
              <Text
                width="40px"
                textAlign="right"
                fontWeight="bold"
                ml="10px"
                mr="5px"
                mt="1px"
              >
                {sliderValue}
              </Text>
              <Hg width={14} height={17} />
            </Flex>
          </FormControl>
          <FormControl isRequired mt="30px" width="30%">
            <FormLabel fontSize="18px" fontWeight="light">
              Location
            </FormLabel>
            <Select
              placeholder="Select Location"
              size="lg"
              onChange={(e) => setLocation(e.target.value)}
            >
              <option value="r">Remote</option>
              <option value="rp">Remote or In Person</option>
              <option value="p">In Person</option>
            </Select>
            {location && location.indexOf("p") > -1 ? (
              <Input placeholder="City Name" size="lg" mt="10px" />
            ) : null}
          </FormControl>
          <Button
            size="lg"
            type="submit"
            mt="30"
            variant="solid"
            colorScheme={"blue"}
          >
            Exchange!
          </Button>
        </form>
      </Box>
    </Box>
  );
}

export default withHeader(CreateExchange);
